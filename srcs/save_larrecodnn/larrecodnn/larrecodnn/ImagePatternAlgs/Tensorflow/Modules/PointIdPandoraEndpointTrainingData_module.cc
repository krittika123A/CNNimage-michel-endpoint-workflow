...
namespace nnet {
    ...
    float xyz[3]) const
  {
    ...
    // These two lines are the only ones that may need a signature tweak
    // if your local LArSoft release exposes different overloads.
    wireAbs = static_cast<float>(wireReadoutGeom.Plane(planeID).WireCoordinate(point.Y(), point.Z()));
