// Failed to successfully edit the PointIdTrainingData_module myself (this is internally inconsistent)//
...
namespace nnet {
  ...
  class PandoraMichelROITrainingData : public art::EDAnalyzer {
  public:
    struct Config {
      ...
      fhicl::Atom<art::InputTag> PandoraPFParticleLabel{
        Name("PandoraPFParticleLabel"),
        Comment("PFParticle label, usually pandora")
      };

      fhicl::Atom<art::InputTag> PandoraTrackLabel{
        Name("PandoraTrackLabel"),
        Comment("Track label carrying recob::Track and Track<->Hit assns")
      };
      ...
      fhicl::Atom<unsigned int> ROISizeW{
        Name("ROISizeW"),
        Comment("ROI size in wires"),
        48u
      };

      fhicl::Atom<unsigned int> ROISizeD{
        Name("ROISizeD"),
        Comment("ROI size in drift bins / ticks according to TrainingDataAlg convention"),
        48u
      };

      fhicl::Atom<bool> SaveCompatHistograms{
        Name("SaveCompatHistograms"),
        Comment("Write raw/deposit/pdg TH2 triplets for each ROI"),
        true
      };

      fhicl::Atom<bool> SaveROITree{
        Name("SaveROITree"),
        Comment("Write metadata tree for each ROI"),
        true
      };

      fhicl::Atom<bool> UseBothEndpoints{
        Name("UseBothEndpoints"),
        Comment("Dump both start and end ROIs"),
        true
      };

      fhicl::Atom<double> TruthDepositThreshold{
        Name("TruthDepositThreshold"),
        Comment("Deposit threshold below which truth labels are zeroed"),
        2.0e-5
      };

      fhicl::Atom<int> TruthCentreRadiusW{
        Name("TruthCentreRadiusW"),
        Comment("Half-width of centre window for endpoint truth matching"),
        6
      };
      ...
      fhicl::Atom<int> TruthMichelMinPixels{
        ...
        Comment("Minimum Michel-labelled pixels inside centre window"),
        3
      };

      fhicl::Atom<int> TruthMuonMinPixels{
        Name("TruthMuonMinPixels"),
        Comment("Minimum muon-labelled pixels in ROI"),
        5
      };
      ...
      // fhicl::Atom<bool> DumpStartPatches{Name("DumpStartPatches"),
      //                                     Comment("Dump start-endpoint patches"), true};
    ...
    explicit PandoraMichelROITrainingData(Parameters const& config);
  ...
  private:
    ...
    art::InputTag fPandoraPFParticleLabel;
    ...
    std::vector<int> fSelectedViews;
    unsigned int fROISizeW;
    unsigned int fROISizeD;
    bool fSaveCompatHistograms;
    bool fSaveROITree;
    ...
    int fTrackKey{}, fEndpoint{}, fCryo{}, fTPC{}, fPlane{};
    float fCenterWire{}, fCenterTick{};
    float fStartXYZ[3]{}, fEndXYZ[3]{};
    int fTruthHasMichel{}, fTruthMichelPixels{}, fTruthMuonPixels{};
    std::vector<float> fRawPatch, fDepositPatch;
    std::vector<int>   fPDGPatch;
    
  };

  //-----------------------------------------------------------------------
  PandoraMichelROITrainingData::PandoraMichelROITrainingData(PandoraMichelROITrainingData::Parameters const& config)
    ...
    , fDriftWindowForROITicks(config().DriftWindowForROITicks())
    , fPatchW(config().Patch_size_w())
    , fPatchD(config().Patch_size_d())
    ...
    , fDumpStartPatches(config().DumpStartPatches())
    , fDumpEndPatches(config().DumpEndPatches())
  ...
  //-----------------------------------------------------------------------
  void PandoraMichelROITrainingData::beginJob()
  {
  ...
  //-----------------------------------------------------------------------
  void PandoraMichelROITrainingData::endJob()
  {
  ...
  //-----------------------------------------------------------------------
  void PandoraMichelROITrainingData::analyze(const art::Event& event)
  {
  ...
  } // PandoraMichelROITrainingData::analyze()

  //-----------------------------------------------------------------------
  int PandoraMichelROITrainingData::WeightedFit(int n,
                                       std::vector<double> const& x,
                                       std::vector<double> const& y,
                                       std::vector<double> const& w,
                                       double* params) const
  ...
  DEFINE_ART_MODULE(PandoraMichelROITrainingData)
