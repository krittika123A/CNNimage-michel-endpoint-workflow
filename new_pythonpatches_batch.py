...
def main(argv):
    ...
    if INPUT_TYPE == "root":
        ...
        # Old behavior ignored --input and always scanned CNNimage:
        # INPUT_DIR = "/exp/dune/app/users/kadhikar/CNNimage"
        # Keep the INPUT_DIR selected from --input/config above so each batch
        # invocation can isolate one reco_hist file.
    ...
    # Old behavior ignored --output and always overwrote files in CNNimage:
    # outdir = "/exp/dune/app/users/kadhikar/CNNimage"
    # Keep the naming convention, but honor --output/config for independent shards.
    outdir = OUTPUT_DIR
