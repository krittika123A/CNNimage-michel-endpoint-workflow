...
#from root_numpy import hist2array  ##not using python2 anymore
...
def get_data(folder, fname, drift_margin = 0, crop = True, blur = None, white_noise = 0, coherent_noise = 0):
    ...
    try:
        if isinstance(folder, TFile):  # read from ROOT file
            #A_raw     = hist2array(folder.Get(fname + '_raw')).astype(np.float32)
            #A_deposit = hist2array(folder.Get(fname + '_deposit')).astype(np.float32)
            ##A_pdg     = hist2array(folder.Get(fname + '_pdg')).astype(np.float32)
            #A_pdg = np.nan_to_num(A_pdg)        # remove NaNs
            #A_pdg = A_pdg.astype(np.int32)      # convert to int

            A_raw     = hist2array(folder.Get(fname + '_raw')).astype(np.float32)
            A_deposit = hist2array(folder.Get(fname + '_deposit')).astype(np.float32)
            A_pdg     = hist2array(folder.Get(fname + '_pdg')).astype(np.int32)
