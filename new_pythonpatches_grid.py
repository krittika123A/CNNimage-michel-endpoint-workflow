...
def main(argv):
    ...
    parser.add_argument(
        '--output-prefix',
        help="Prefix for output numpy files. Grid jobs use this to write unique shard names.",
        default=None
    )
    args = parser.parse_args()
    ...
    if INPUT_TYPE == "root":
        ...
        # Grid copy: use the input directory passed with -i. The original script's
        # hard-coded dunegpvm path is intentionally commented out here so each worker
        # processes only the reco_hist*.root file staged into its private directory.
        # INPUT_DIR = "/exp/dune/app/users/kadhikar/CNNimage"
    ...
    # Grid copy: write into the output directory passed with -o. The original
    # hard-coded dunegpvm path is intentionally commented out here so JustIN can
    # collect one unique numpy shard pair from each worker.
    # outdir = "/exp/dune/app/users/kadhikar/CNNimage"
    outdir = OUTPUT_DIR
    os.makedirs(outdir, exist_ok=True)

    if output_prefix is None:
        output_prefix = f"db_view_{selected_view_idx}"
    else:
        output_prefix = f"{output_prefix}_view_{selected_view_idx}"

    x_path = os.path.join(outdir, f"{output_prefix}_x.npy")
    y_path = os.path.join(outdir, f"{output_prefix}_y.npy")
